...
export class DeliveryService {
  ...
  async assignOrder(orderId: string, deliveryPartnerId: string) {
    ...
    try {
      ...
      const partnerDetails = await this.userProxyService.fetchDeliveryPartnerDetails(deliveryPartnerId);
      if(!partnerDetails.availability){
        throw new ForbiddenException(`Delivery partner ${deliveryPartnerId} is currently unavailable`);
      }
  ...
  async registerDeliveryAssignTask(orderReceived:any) {
    ...
    this.logger.log(`Created delivery task ${deliveryTask.id} for order ${orderReceived.id}`);
    return deliveryTask;
  }
}
